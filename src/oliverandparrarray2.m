function [Fitting,naughty_indents_list,red_indents_list] = oliverandparrarray2(base_file_directory,load_displacement_data,epsilon,samplepossionratio,tolerance,cutofdatavalue,cutofunloadingtoplim,cutofunloadingbottomlim,naughty_indents_list,red_indents_list)
...
for i=0:1:noofindents-1 % loop for each of the indents with zero corrections
...
%         end
    ...
    %Plot fit with data.
     ...
     hold on
        ...
        derivativeofpowerlaw=differentiate(fitresult,unloadingh);
...
%        hold on
    ...
    hold on
     ...
     if values_of_H_and_E(j,4)<0 %write this in as a dodgy indent
         values_of_H_and_E(j,4)= NaN;
         values_of_H_and_E(j,3)= NaN;
         values_of_H_and_E(j,2)= NaN;
         values_of_H_and_E(j,5)= NaN;
